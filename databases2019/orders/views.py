    ...
    if request.method == 'POST':
        ...
        if order_details_form.is_valid():            
            ...
            if order_details.quantity > (in_stock - on_order):
                ...
                order_details.save()
                total_price = unit_price * order_details.quantity * decimal.Decimal(1 - order_details.discount)
                messages.success(request, f'Products of total value {total_price} GDB added to order no. {order_id}')
                return redirect('/orders/add_order_details/' + str(order_id) + '/')
    else:
